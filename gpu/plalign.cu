...
// Long-task kernel: the persistent work-stealing fused kernel (GMEM, no dynamic
// shared memory).
...
#ifdef NO_BATCH_SCHED
    // Batch-level scheduler OFF (ablation): collapse all long tasks into a single
    // class (A).  No analytical A/B/C partitioning; the single-pool path below
    // then uses the larger bt_p pool for every task, and slot clamps keep it
    // correct (just fewer concurrent slots -> slower, which is the point).
    const size_t THRESH_B_PRE = SIZE_MAX;
    const size_t THRESH_C_PRE = SIZE_MAX;
#else
...
#ifdef NO_BATCH_SCHED
    c_concurrent = false;  // batch-level scheduler OFF: no concurrent-C overlap
#endif
    ...
    for (int phase = 0; phase < 2; phase++) {
            ...
            while (a_done < nA || b_done < nB || c_done < nC) {
                ...
                auto mc_map = [&](const std::vector<int>& vidx, int start, int count, int base_i) {
                    for (int i = 0; i < count; i++) {
                        ...
                        // reach_end exactly as CPU ksw2 (ksw2_extd2_sse.c:391-397):
                        // extension (EXTZ_ONLY) iff !zdropped && mqe+end_bonus>max;
                        // gap-fill (global) never sets it.
                        if (tasks[tidx].flag & KSW_EZ_EXTZ_ONLY) {
                            tasks[tidx].reach_end =
                                (!h_zdropped[gi] &&
                                 tasks[tidx].mqe + tasks[tidx].end_bonus > tasks[tidx].score);
                        } else {
                            tasks[tidx].reach_end = 0;
                        }
        ...
        while (tasks_processed_in_phase < n_tasks_in_phase) {
            ...
            for (int i = 0; i < batch_size; i++) {
                ...
                // extension (EXTZ_ONLY) iff !zdropped && mqe+end_bonus>max;
                // gap-fill (global) never sets it.
                if (tasks[task_idx].flag & KSW_EZ_EXTZ_ONLY) {
                    tasks[task_idx].reach_end =
                        (!h_zdropped[i] &&
                         tasks[task_idx].mqe + tasks[task_idx].end_bonus > tasks[task_idx].score);
                } else {
                    tasks[task_idx].reach_end = 0;
                }
