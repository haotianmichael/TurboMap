                                       ...
                                       const char *skip_reads)
{
    ...
    for (int i = 0; i < gpu_batch->n_tasks; i++) {
        ...
        if (has_valid_alignment && task->n_cigar > 0) {
            ...
            // GAP_FILL backtracks from the corner (full gap) unless z-dropped, so this
            // normally adds nothing.  max_q/max_t < 0 is the APPROX_MAX sentinel meaning
            // "corner backtrack, full gap already covered" — and since the two-pass clears
            // APPROX_MAX from the flag, that sentinel is the reliable signal.  Using max_q=-1
            // here would append a second full copy of the gap (CIGAR 2x -> qe1 > qlen).
            if (task->task_type == GPU_TASK_GAP_FILL && has_valid_alignment && !task->zdropped
                    && !(task->flag & KSW_EZ_APPROX_MAX)
                    && task->max_q >= 0 && task->max_t >= 0) {
...
// [ZDROP_DEBUG] Whole-run counter of z-drop detections that require a second
// (exact, non-APPROX) GPU alignment pass.  Incremented once per GAP_FILL task
// for which mm_test_zdrop() fires (see the retry-collection loop below).  The
// total is printed to stderr once at process exit.
static unsigned long long g_zdrop_trigger_count = 0;  // positive detections (exact-align retries)
static unsigned long long g_zdrop_check_count   = 0;  // GAP_FILL tasks z-drop-checked (denominator)
static unsigned long long g_chain_dropped_reads = 0;  // reads dropped: exceed GPU chain capacity (fit==0)

// Record details of the first ZDROP_LEN_CAP z-drop-triggered tasks so the
// SPARSITY of z-drop is visible.  Indexed by the 0-based trigger ordinal:
//   g_zdrop_task_id  : this task's global ordinal among ALL GAP_FILL tasks
//                      z-drop-checked this run (1-based) -> sparse over
//                      1..g_zdrop_check_count, the x-axis that shows sparsity
//   g_zdrop_read_name: the read this task belongs to (real read id; copied,
//                      since ctx->name is only valid for the batch lifetime)
//   g_zdrop_task_len : max(qlen, tlen) of the GAP_FILL DP (re-alignment size)
#define ZDROP_LEN_CAP 1000
static unsigned long long g_zdrop_task_id[ZDROP_LEN_CAP];
static char               g_zdrop_read_name[ZDROP_LEN_CAP][256];
static int32_t            g_zdrop_task_len[ZDROP_LEN_CAP];

static void mm_zdrop_count_report(void)
{
    unsigned long long n      = __atomic_load_n(&g_zdrop_trigger_count, __ATOMIC_RELAXED);
    unsigned long long checks = __atomic_load_n(&g_zdrop_check_count,   __ATOMIC_RELAXED);
    fprintf(stderr,
            "[ZDROP_DEBUG] z-drop detections (exact-align retries this run): %llu"
            "   (out of %llu GAP_FILL tasks checked)\n", n, checks);
    fprintf(stderr,
            "[CHAIN_DEBUG] reads dropped (exceeded GPU chain capacity, fit==0): %llu\n",
            __atomic_load_n(&g_chain_dropped_reads, __ATOMIC_RELAXED));

    // Dump the first up-to-1000 triggered tasks. Columns:
    //   task_id   : global GAP_FILL z-drop-check ordinal (sparse over 1..checks)
    //   read_name : the read this task belongs to
    //   task_len  : max(qlen, tlen) of the gap DP
    unsigned long long m = n < ZDROP_LEN_CAP ? n : ZDROP_LEN_CAP;
    const char *path = getenv("ZDROP_LEN_FILE");
    if (!path) path = "zdrop_task_len.txt";
    FILE *fp = fopen(path, "w");
    if (fp) {
        fprintf(fp, "task_id\tread_name\ttask_len\n");
        for (unsigned long long i = 0; i < m; i++)
            fprintf(fp, "%llu\t%s\t%d\n",
                    g_zdrop_task_id[i], g_zdrop_read_name[i], g_zdrop_task_len[i]);
        fclose(fp);
        fprintf(stderr, "[ZDROP_DEBUG] wrote %llu z-drop tasks to %s "
                        "(out of %llu checked -> sparsity %.4f%%)\n",
                m, path, checks, checks ? 100.0 * (double)n / (double)checks : 0.0);
    } else {
        fprintf(stderr, "[ZDROP_DEBUG] could not open '%s' for writing\n", path);
    }
}

__attribute__((constructor))
static void mm_zdrop_count_init(void) { atexit(mm_zdrop_count_report); }
                                          ...
                                          char **retry_reads_out)
{
    ...
    // Second-pass GPU retry: mirrors CPU mm_align1 two-pass design.
    // CPU first pass: APPROX_MAX with zdrop enabled, then mm_test_zdrop detects
    // real structural zdrop events.  If detected, second pass runs WITHOUT
    // APPROX_MAX and with the correct zdrop/zdrop_inv threshold.
    ...
    // the first-pass CIGAR in-place; process_results then sees final CIGARs only.
    {
        ...
        // Collect tasks needing second pass (upper bound: all GAP_FILL tasks).
        int max_retry = gpu_batch->n_tasks;
        gpu_align_task_t *retry_tasks = (gpu_align_task_t*)malloc(max_retry * sizeof(gpu_align_task_t));
        int *retry_orig_idx = (int*)malloc(max_retry * sizeof(int));
        int n_retry = 0;

        for (int i = 0; i < gpu_batch->n_tasks; i++) {
            gpu_align_task_t *task = &gpu_batch->tasks[i];

            if (task->task_type != GPU_TASK_GAP_FILL) continue;
            if (!(task->flag & KSW_EZ_APPROX_MAX)) continue;

            // Clear APPROX_MAX now: process_results must not re-run mm_test_zdrop
            // on these tasks — we handle it here for all cases.
            task->flag &= ~KSW_EZ_APPROX_MAX;

            if (task->n_cigar == 0 || task->zdropped) continue;
            ...
            if (tlen_gap <= 0 || !ctx->qseq0[0]) continue;

            uint8_t *tseq_gap = (uint8_t*)kmalloc(km, tlen_gap);
            mm_idx_getseq(mi, task->task_ctx.rid, (uint32_t)ref_rs, (uint32_t)ref_re, tseq_gap);
            // Use the same qseq orientation as the GPU kernel received.
            uint8_t *qseq_gap = (opt->flag & MM_F_QSTRAND) ? ctx->qseq0[0] + ref_qs
                                                            : ctx->qseq0[rev]  + ref_qs;
            uint32_t *first_cigar = gpu_batch->cigar_buffer + task->cigar_offset;

            int zdrop_code = mm_test_zdrop(km, opt, qseq_gap, tseq_gap,
                                           task->n_cigar, first_cigar, mat);
            kfree(km, tseq_gap);

            // [ZDROP_DEBUG] count every z-drop check (check_id = this task's
            // 1-based global ordinal among checked GAP_FILL tasks), and every
            // positive detection that forces a second exact-alignment pass.
            unsigned long long check_id =
                __atomic_fetch_add(&g_zdrop_check_count, 1ULL, __ATOMIC_RELAXED) + 1;

            if (zdrop_code != 0) {
                // tid = unique 0-based ordinal of this trigger (race-free).
                unsigned long long tid =
                    __atomic_fetch_add(&g_zdrop_trigger_count, 1ULL, __ATOMIC_RELAXED);
                if (tid < ZDROP_LEN_CAP) {
                    g_zdrop_task_id[tid]  = check_id;
                    g_zdrop_task_len[tid] =
                        task->qlen > task->tlen ? task->qlen : task->tlen;
                    snprintf(g_zdrop_read_name[tid], sizeof(g_zdrop_read_name[tid]),
                             "%s", ctx->name ? ctx->name : "(null)");
                }
                // Build second-pass retry task: same sequences/buffer offsets,
                // zdrop enabled (or zdrop_inv for inversion), no APPROX_MAX.
                retry_tasks[n_retry] = *task;
                retry_tasks[n_retry].zdrop = (zdrop_code == 2) ? (int32_t)opt->zdrop_inv
                                                                : (int32_t)opt->zdrop;
                // flag already has APPROX_MAX cleared (done above); keep as-is.
                // Reset result fields so the GPU kernel writes fresh output.
                retry_tasks[n_retry].n_cigar   = 0;
                retry_tasks[n_retry].zdropped  = 0;
                retry_tasks[n_retry].score     = 0;
                retry_tasks[n_retry].max_q     = retry_tasks[n_retry].max_t = 0;
                retry_tasks[n_retry].mqe       = retry_tasks[n_retry].mqe_t = -1;
                retry_tasks[n_retry].mte       = retry_tasks[n_retry].mte_q = -1;
                retry_tasks[n_retry].reach_end = 0;

                retry_orig_idx[n_retry] = i;
                n_retry++;
            }
        }
    ...
    if (!is_sr && ctx->qseq0[0] && ctx->a) {
        ...
        while (ireg < ctx->n_regs) {
            ...
            if (rp->p == NULL && rp->cnt > 0) {
                ...
                if (r2_cpu.cnt > 0) {
                    // Insert the remainder at ireg+1; the bottom ++ireg then lands
                    // ON it so it is re-aligned next iteration — matching CPU
                    // mm_align_skeleton (align.c) which re-aligns every z-drop
                    // remainder to a fixpoint.  (Removing the previous extra ireg++
                    // that skipped the freshly inserted remainder.)
...
// ──────────────────────────────────────────────────────────────────────
// Cross-batch z-drop remainder (r2) carry pool
//
// z-drop splits are sparse, so re-aligning each batch's r2 remainders
// immediately submits tiny GPU batches (dozens of tasks) that badly
// underutilize the device.  Instead we DEFER r2 remainders into a per-stream
// pool and re-align many batches' worth together, keeping GPU batches large.
// Only the per-read state that lives in the recycled batch km — the anchor
// array `a` and encoded query `qseq0` — is deep-copied into the pool's own km;
// regs0 and the partial CIGARs (r->p) are heap-allocated (calloc/realloc in
...
typedef struct {
    ...
    read_align_ctx_t *ctxs;    // carried align context (regs0 heap; a/qseq0 in pool km)
    int               count;
    int               cap;
    void             *km;      // dedicated km for deep-copied a/qseq0/qlens
} r2_carry_pool_t;

static void r2_pool_init(r2_carry_pool_t *p) {
    p->reads = NULL; p->ctxs = NULL; p->count = 0; p->cap = 0; p->km = km_init();
}
static void r2_pool_reset(r2_carry_pool_t *p) {
    // regs0 / r->p ownership has been transferred to s->reg by finalize, and
    // a / qseq0 / qlens live in p->km — so dropping p->km frees exactly the
    // deep copies and nothing that is still in use.
    km_destroy(p->km); p->km = km_init();
    p->count = 0;
}
static void r2_pool_destroy(r2_carry_pool_t *p) {
...
static void r2_pool_carry(r2_carry_pool_t *pool, const chain_read_t *read_,
    ...
    chain_read_t     *dr = &pool->reads[pool->count];
    read_align_ctx_t *dc = &pool->ctxs[pool->count];
    *dr = *read_;   // scalars + seq.name (chunk-lived) + seg ids
    *dc = *ctx;     // regs0 (heap), n_regs, n_a, qlen, name; a/qseq0 reset below

    dr->qlens = (int*)kmalloc(pool->km, sizeof(int) * read_->n_seg);
    memcpy(dr->qlens, read_->qlens, sizeof(int) * read_->n_seg);
    dr->qseqs = NULL; dr->u = NULL; dr->mini_pos = NULL;
    dr->a_full = NULL; dr->n_full = 0;  // batch-km pointers; not used past finalize
...
static void* drain_worker_fn(void *arg) {
    ...
    while (1) {
        ...
        if (slot->shutdown) {
            ...
            // Final consolidation: re-align any r2 remainders still pending in
    ...
    while (1) {
        ...
        if (is_full && acc_batch.count > 0) {
            ...
            if (fit == 0) {
                ...
                __atomic_fetch_add(&g_chain_dropped_reads,
                                   (unsigned long long)orig_count, __ATOMIC_RELAXED);
