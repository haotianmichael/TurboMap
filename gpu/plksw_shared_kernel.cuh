...
/* ============================================================================
 *
 *           PARTIAL-SHARED LONG KERNEL  (3 hottest deltas in shared)
 *
 * For GPUs with smaller per-block shared budget (V100 = 96 KB) where
 * 6×(tlen+1) doesn't fit but 3×(tlen+1) does.
 *
 * Layout:
 *   shared : x, v, x2  (3 hottest — each accessed 3× per cell:
 *                       read at t, read at t-1, write at t)
 *   global : u, y, y2  (each accessed 2× per cell: read at t, write at t)
 *           + H, qr, target, bt_p, off, off_end  (same as legacy)
 *
 * Per cell, this eliminates 9 of 15 global delta accesses (60%).  Expected
 * speedup ~2× over legacy on the bottleneck super-long batch.
 *
 * Capacity:
 *   3 × (tlen+1) ≤ shared budget  →  tlen ≤ ~32k for V100, ~54k for A100.
 *   Comfortably fits the user's super-long tasks (tlen ≈ 22kbp).
 *
 * temp_buffer layout matches the LEGACY kernel exactly.  v/x/x2 regions in
 * global are unused (wasted) but indexing v/x/x2 via shared is what saves
 * the latency.  Total wasted global = 3 × (tlen+1) per slot — small.
 * ============================================================================ */
__global__ void ksw_long_shared3_kernel(
    int      *d_task_counter,
    uint32_t *packed_query_batch,
    uint32_t *packed_ref_batch,
    uint32_t *query_batch_lens,
    ...
    int       max_backtrack_size,
    int       max_antidiag,
    void     *d_temp_buffer,        /* legacy layout — we just skip v/x/x2 */
    int      *d_flag,
    int32_t  *d_bw,
    size_t    temp_per_task,
    int       n_tasks,
    ...
    int       max_tlen_per_block)   /* shared-mem stride per delta array  */
{
    ...
    /* Per-block shared layout: 3 contiguous arrays of size (max_tlen+1) bytes. */
    const int stride = max_tlen_per_block + 1;
    int8_t *x_arr  = shared_buf + 0 * stride;
    int8_t *v_arr  = shared_buf + 1 * stride;
    int8_t *x2_arr = shared_buf + 2 * stride;
    ...
    while (true) {
        ...
        /* Per-slot global temp buffer — LEGACY layout (v/x/x2 region unused). */
        char *task_buf = (char*)d_temp_buffer + (size_t)slot_id * temp_per_task;
        size_t off_b = 0;
        int32_t *H = (int32_t*)(task_buf + off_b); off_b += (size_t)tlen * sizeof(int32_t);
        int8_t *u_arr = (int8_t*)(task_buf + off_b); off_b += (size_t)(tlen + 1);
        off_b += (size_t)(tlen + 1);                 /* skip v_arr (in shared) */
        off_b += (size_t)(tlen + 1);                 /* skip x_arr (in shared) */
        int8_t *y_arr = (int8_t*)(task_buf + off_b); off_b += (size_t)(tlen + 1);
        off_b += (size_t)(tlen + 1);                 /* skip x2_arr (in shared) */
        int8_t *y2_arr = (int8_t*)(task_buf + off_b); off_b += (size_t)(tlen + 1);
        uint8_t *qr     = (uint8_t*)(task_buf + off_b); off_b += (size_t)qlen;
        uint8_t *target = (uint8_t*)(task_buf + off_b);

        /* Init: shared deltas + global deltas + H. */
        int8_t neg_qe  = (int8_t)(-(int)(q + e));
        int8_t neg_qe2 = (int8_t)(-(int)(q2 + e2));
        for (int i = lane_id; i < tlen; i += WARP_SIZE) {
            H[i]      = KSW_NEG_INF;
        ...
        for (int r = 0; r < qlen + tlen - 1; r++) {
            ...
            for (int batch_start = 0; batch_start < band_size; batch_start += WARP_SIZE) {
                ...
                if (active) {
                    ...
                    /* Pre-load OLD values: x/v/x2 from SHARED, u/y/y2 from GLOBAL. */
                    old_x_at_t  = x_arr[t];
                    old_v_at_t  = v_arr[t];
                    old_x2_at_t = x2_arr[t];
                ...
                if (active) {
                    ...
                    /* Writes: x, v, x2 → shared; u, y, y2 → global. */
                    u_arr[t]  = new_u;       /* global */
                    v_arr[t]  = new_v_out;   /* shared */
                    x_arr[t]  = new_x;       /* shared */
                    y_arr[t]  = new_y;       /* global */
                    x2_arr[t] = new_x2;      /* shared */
                    y2_arr[t] = new_y2;      /* global */
        ...
        /* Fused backtrack walk — bt_p reads from global, identical to legacy. */
        if (cigar_buffer && with_cigar && lane_id == 0 &&
            backtrack_q >= 0 && backtrack_t >= 0) {
            int i = backtrack_t, j = backtrack_q;
            int state = 0;
