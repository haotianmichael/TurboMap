...
// Kernel DP path (compile-time exclusive):
//   DPX_KSW   → Hopper int16x2 + __viaddmax_s16x2 (sm_90+), GMEM fused persistent.
//   HALF2_KSW → FP16x2 half2 (Volta+), GMEM fused persistent (default).
#if defined(DPX_KSW)
  #if defined(__CUDA_ARCH__) && (__CUDA_ARCH__ < 900)
    #error "DPX_KSW requires sm_90 (H100). Compile with GPUARCH=sm_90."
  #endif
  #include "plksw_dpx_core.cuh"
#elif defined(HALF2_KSW)
  #include "plksw_half2_core.cuh"
#endif
    ...
    while (true) {
        ...
        for (int r = 0; r < qlen + tlen - 1; r++) {
...
#if defined(DPX_KSW)
            // ---- DPX path: each lane processes 2 cells per iteration (stride = 64) ----
            //      Fused add+max via __viaddmax_s16x2 (Hopper sm_90+), GMEM.
            unsigned dpx_gO     = dpx_bcast((short)q);
            unsigned dpx_gOL    = dpx_bcast((short)q2);
            unsigned dpx_negOE  = dpx_bcast((short)(-(int)qe));
            unsigned dpx_negOEL = dpx_bcast((short)(-(int)qe2));
            unsigned dpx_scMch  = dpx_bcast((short)sc_mch);

            for (int batch_start = 0; batch_start < band_size; batch_start += WARP_SIZE * 2) {
                int idx0 = batch_start + 2 * lane_id;
                int idx1 = idx0 + 1;
                bool active0 = (idx0 < band_size);
                bool active1 = (idx1 < band_size);
                int t0 = st0 + idx0;
                int t1 = st0 + idx1;

                int8_t old_x0 = 0, old_v0 = 0, old_x20 = 0;
                int8_t old_x1 = 0, old_v1 = 0, old_x21_h = 0;
                if (active0) { old_x0 = x_arr[t0]; old_v0 = v_arr[t0]; old_x20 = x2_arr[t0]; }
                if (active1) { old_x1 = x_arr[t1]; old_v1 = v_arr[t1]; old_x21_h = x2_arr[t1]; }

                int8_t lx0, lv0, lx20;
                if (lane_id == 0) {
                    lx0 = batch_x1_boundary; lv0 = batch_v1_boundary; lx20 = batch_x21_boundary;
                } else if (active0) {
                    lx0 = x_arr[t0 - 1]; lv0 = v_arr[t0 - 1]; lx20 = x2_arr[t0 - 1];
                } else { lx0 = lv0 = lx20 = 0; }
                int8_t lx1 = old_x0, lv1 = old_v0, lx21 = old_x20;

                int8_t up_u0  = active0 ? u_arr[t0]  : 0;
                int8_t up_y0  = active0 ? y_arr[t0]  : 0;
                int8_t up_y20 = active0 ? y2_arr[t0] : 0;
                int8_t up_u1  = active1 ? u_arr[t1]  : 0;
                int8_t up_y1  = active1 ? y_arr[t1]  : 0;
                int8_t up_y21 = active1 ? y2_arr[t1] : 0;

                int8_t sc0 = 0, sc1 = 0;
                if (active0) {
                    int qi = r - t0;
                    if (qi >= 0 && qi < qlen && t0 >= 0 && t0 < tlen)
                        sc0 = dp_compute_score(qr[qlen - 1 - qi], target[t0], device_mat, m, sc_N);
                }
                if (active1) {
                    int qi = r - t1;
                    if (qi >= 0 && qi < qlen && t1 >= 0 && t1 < tlen)
                        sc1 = dp_compute_score(qr[qlen - 1 - qi], target[t1], device_mat, m, sc_N);
                }

                unsigned xv1  = dpx_from_i8x2(lx0,   lx1);
                unsigned vv1  = dpx_from_i8x2(lv0,   lv1);
                unsigned x2v1 = dpx_from_i8x2(lx20,  lx21);
                unsigned uup  = dpx_from_i8x2(up_u0, up_u1);
                unsigned yyp  = dpx_from_i8x2(up_y0, up_y1);
                unsigned y2yp = dpx_from_i8x2(up_y20,up_y21);
                unsigned ssc  = dpx_from_i8x2(sc0,   sc1);

                unsigned nu, nv, nx, ny, nx2, ny2;
                uint8_t d0 = 0, d1 = 0;
                dp_cell_pair_dpx(xv1, vv1, x2v1, uup, yyp, y2yp, ssc,
                                 dpx_gO, dpx_gOL, dpx_negOE, dpx_negOEL, dpx_scMch,
                                 (int)q, (int)q2, (bool)with_cigar, (bool)right_align,
                                 &nu, &nv, &nx, &ny, &nx2, &ny2, &d0, &d1);

                if (active0) {
                    u_arr[t0]  = dpx_to_i8_lo(nu);
                    v_arr[t0]  = dpx_to_i8_lo(nv);
                    x_arr[t0]  = dpx_to_i8_lo(nx);
                    y_arr[t0]  = dpx_to_i8_lo(ny);
                    x2_arr[t0] = dpx_to_i8_lo(nx2);
                    y2_arr[t0] = dpx_to_i8_lo(ny2);
                    if (pr != NULL) pr[idx0] = d0;
                }
                if (active1) {
                    u_arr[t1]  = dpx_to_i8_hi(nu);
                    v_arr[t1]  = dpx_to_i8_hi(nv);
                    x_arr[t1]  = dpx_to_i8_hi(nx);
                    y_arr[t1]  = dpx_to_i8_hi(ny);
                    x2_arr[t1] = dpx_to_i8_hi(nx2);
                    y2_arr[t1] = dpx_to_i8_hi(ny2);
                    if (pr != NULL) pr[idx1] = d1;
                }

                if (batch_start + WARP_SIZE * 2 < band_size) {
                    batch_x1_boundary  = __shfl_sync(0xffffffff, old_x1,    WARP_SIZE - 1);
                    batch_v1_boundary  = __shfl_sync(0xffffffff, old_v1,    WARP_SIZE - 1);
                    batch_x21_boundary = __shfl_sync(0xffffffff, old_x21_h, WARP_SIZE - 1);
                }
                __syncwarp();
            } // End DPX batch loop
#elif defined(HALF2_KSW)
            // ---- half2 path: each lane processes 2 cells per iteration (stride = 64) ----
            half2 h2_gO     = __half2half2(i8_to_h(q));
            half2 h2_gOL    = __half2half2(i8_to_h(q2));
            half2 h2_gOE    = __half2half2(i8_to_h(qe));
            half2 h2_gOEL   = __half2half2(i8_to_h(qe2));
            half2 h2_negOE  = __half2half2(i8_to_h((int8_t)(-(int)qe)));
            half2 h2_negOEL = __half2half2(i8_to_h((int8_t)(-(int)qe2)));
            half2 h2_scMch  = __half2half2(i8_to_h(sc_mch));
            ...
            for (int batch_start = 0; batch_start < band_size; batch_start += WARP_SIZE * 2) {
                ...
                // Save OLD r-1 values (read BEFORE any write in this batch)
                int8_t old_x0 = 0, old_v0 = 0, old_x20 = 0;
                int8_t old_x1 = 0, old_v1 = 0, old_x21_h = 0;
                if (active0) { old_x0 = x_arr[t0]; old_v0 = v_arr[t0]; old_x20 = x2_arr[t0]; }
                if (active1) { old_x1 = x_arr[t1]; old_v1 = v_arr[t1]; old_x21_h = x2_arr[t1]; }

                // Left neighbors (cell idx0 ← t0-1 or boundary; cell idx1 ← cell idx0's OLD value)
                int8_t lx0, lv0, lx20;
                if (lane_id == 0) {
                    lx0 = batch_x1_boundary; lv0 = batch_v1_boundary; lx20 = batch_x21_boundary;
                } else if (active0) {
                ...
                // Up neighbors (same column t, r-1)
                int8_t up_u0 = active0 ? u_arr[t0] : 0;
                int8_t up_y0 = active0 ? y_arr[t0] : 0;
                int8_t up_y20 = active0 ? y2_arr[t0] : 0;
                int8_t up_u1 = active1 ? u_arr[t1] : 0;
                ...
                // Pack and run helper
                half2 xv1  = __halves2half2(i8_to_h(lx0),   i8_to_h(lx1));
                half2 vv1  = __halves2half2(i8_to_h(lv0),   i8_to_h(lv1));
                half2 x2v1 = __halves2half2(i8_to_h(lx20),  i8_to_h(lx21));
                half2 uup  = __halves2half2(i8_to_h(up_u0), i8_to_h(up_u1));
                half2 yyp  = __halves2half2(i8_to_h(up_y0), i8_to_h(up_y1));
                half2 y2yp = __halves2half2(i8_to_h(up_y20),i8_to_h(up_y21));
                half2 ssc  = __halves2half2(i8_to_h(sc0),   i8_to_h(sc1));

                half2 nu, nv, nx, ny, nx2, ny2;
                uint8_t d0 = 0, d1 = 0;
                dp_cell_pair_half2(xv1, vv1, x2v1, uup, yyp, y2yp, ssc,
                                   h2_gO, h2_gOL, h2_gOE, h2_gOEL,
                                   h2_negOE, h2_negOEL, h2_scMch,
                                   (int)q, (int)q2, (bool)with_cigar, (bool)right_align,
                                   &nu, &nv, &nx, &ny, &nx2, &ny2, &d0, &d1);

                if (active0) {
                    u_arr[t0]  = h_to_i8(__low2half(nu));
                    v_arr[t0]  = h_to_i8(__low2half(nv));
                    x_arr[t0]  = h_to_i8(__low2half(nx));
                    y_arr[t0]  = h_to_i8(__low2half(ny));
                    x2_arr[t0] = h_to_i8(__low2half(nx2));
                    y2_arr[t0] = h_to_i8(__low2half(ny2));
                    if (pr != NULL) pr[idx0] = d0;
                }
                if (active1) {
                    u_arr[t1]  = h_to_i8(__high2half(nu));
                    v_arr[t1]  = h_to_i8(__high2half(nv));
                    x_arr[t1]  = h_to_i8(__high2half(nx));
                    y_arr[t1]  = h_to_i8(__high2half(ny));
                    x2_arr[t1] = h_to_i8(__high2half(nx2));
                    y2_arr[t1] = h_to_i8(__high2half(ny2));
                    if (pr != NULL) pr[idx1] = d1;
                }

                // Boundary shuffle for next batch's lane 0:
                //   next batch's first cell t = st0 + batch_start + 64; it needs OLD x/v/x2
                //   at t-1 = st0 + batch_start + 63 = lane 31's idx1 cell.
                if (batch_start + WARP_SIZE * 2 < band_size) {
                    batch_x1_boundary  = __shfl_sync(0xffffffff, old_x1,    WARP_SIZE - 1);
                    batch_v1_boundary  = __shfl_sync(0xffffffff, old_v1,    WARP_SIZE - 1);
                    batch_x21_boundary = __shfl_sync(0xffffffff, old_x21_h, WARP_SIZE - 1);
