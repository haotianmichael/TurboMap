...
__device__ int32_t warp_reduce_max(int32_t val, unsigned mask) {
    // Step 1: 所有线程执行相同的shuffle模式
    #pragma unroll
    for (int offset = 16; offset > 0; offset /= 2) {
        int32_t tmp = __shfl_down_sync(mask, val, offset);
        val = max(val, tmp);
    }
			...
			if (!terminated) {
				for (diag_idx = i<<3; diag_idx < last_diag; diag_idx++) {
					if (diag_idx <prev_max_score) {
						...
						int32_t my_val = antidiag_max[(m<<5)+real_warp_id];
						temp = warp_reduce_max(my_val, same_threads);
