...
void plmem_config_batch(cJSON *json, int *num_stream_,
    ...
    *max_read_ = (int)(*max_total_n_ / avg_read_n);
    ...
    // Optional manual override of max_read (per-microbatch read cap).  Lower it to
    // force more microbatches (better seed→align→finalize pipeline overlap) on
    // high-VRAM GPUs; leave unset for the VRAM-derived automatic value.
    cJSON *max_read_json = cJSON_GetObjectItem(json, "max_read");
    if (max_read_json && max_read_json->valueint > 0)
