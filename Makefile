...
CPPFLAGS=	-DHAVE_KALLOC -DGPU_PIPELINE
...
# Make compares file timestamps, not compile flags, so toggling RAW=1 / DPX=1
...
# RAW_EXT_ONLY / DPX_KSW depend on it so they rebuild exactly when needed.
